...
<main class="p-6 sm:p-10 space-y-6">
          ...
          <div class="relative bg-white rounded-lg shadow ">
              ...
              <div class="p-4 md:p-5 space-y-4">
                  ...
                  <section class="bg-gray-50 ">
                    <form class="space-y-4 md:space-y-6" action="/admin/office/register" method="post">
                      @method('POST')
                      @csrf
                      <div>
  ...
  <a href="/admin/dashboard/account/alluser?aria-data=true" type="button" class="border-solid border-2  text-gray-900 bg-white hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center  me-2 mb-2">
   All User
    </a>
        ...
        <tbody>
          @foreach ($officer as $of )
            <tr class="odd:bg-white even:bg-gray-50  border-b ">
                ...
                <td class="px-6 py-4">
                    <button data-modal-target="edit-modal{{$of->id}}" data-modal-toggle="edit-modal{{$of->id}}" type="button" class="font-medium text-blue-600  hover:underline">Edit</button>
                    <a href="/admin/office/un/{{$of->id}}">
                    | &nbsp; unOfficer</a>
                </td>
            </tr>
              ...
              <div class="relative p-4 w-full max-w-2xl max-h-full">
                  ...
                  <div class="relative bg-white rounded-lg shadow ">
                      ...
                      <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
                          ...
                          <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center " data-modal-hide="edit-modal{{$of->id}}">
                              <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                              </svg>
                              <span class="sr-only">Close modal</span>
                      ...
                      <div class="p-4 md:p-5 space-y-4">
                          <!-- Isi form dari sektion yang diberikan -->
                          <section class="bg-gray-50 ">
                            <form class="space-y-4 md:space-y-6" action="/admin/office/register/edit" method="post">
                              @method('POST')
                              @csrf
                              <input type="hidden" name="key" value="{{$of->id}}">
                              <div>
                                <label for="email" class="block mb-2 text-sm font-medium text-gray-900 "> email</label>
                                <input type="email" name="email" value="{{$of->email}}" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 "  required="">
                            </div>
                            <div>
                                <label for="email" class="block mb-2 text-sm font-medium text-gray-900 ">username</label>
                                <input type="text" value="{{$of->username}}" name="username" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 "  required="">
                            </div>
                            <div>
                                <label for="email" class="block mb-2 text-sm font-medium text-gray-900 ">Full Name</label>
                                <input type="text" value="{{$of->fullname}}" name="fullname" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 "  required="">
                            </div>
                            <div>
                                <label for="password" class="block mb-2 text-sm font-medium text-gray-900 ">Password</label>
                            ...
                            <br>
                                <button type="submit" class="w-full text-white bg-blue-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center ">Confirm</button>
                      ...
                      <!-- Modal footer -->
                      <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b ">
                          <button data-modal-hide="edit-modal{{$of->id}}" type="button" class="ms-3 text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10">Cancel</button>
                      </div>
                  </div>
              </div>
          </div>
